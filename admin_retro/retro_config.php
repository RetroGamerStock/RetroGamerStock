...
if(isset($_SESSION['login']) && isset($_SESSION['mdp']) && $result['membType'] == 2){
	...
	$modif = 'UPDATE config SET nom_site="'.mysql_escape_string($_POST['nom_site']).'", 
							  ...
							  pseudo_admin="'.mysql_escape_string($_POST['ps_admin']).'"';   
								  
		if (!$modif) {
		 die('Requête invalide : ' . mysql_error());
		}
		else{
			$erreur_ok ='<div class="ok">Mise à jour éffectué avec succès. Redirection en cours...</div><script type="text/javascript"> window.setTimeout("location=(\'index.php\');",3000) </script>';		
			} 
	}
}
?>
		...
		<?php if (isset($erreur_ok)) echo $erreur_ok; else if (isset($erreur)) echo $erreur;?>          
            <form action="#" method="post">  
              ...
              <legend>Informations de connexion</legend>   
                      ...
                      <input class="inscription" name="ps_admin" type="text" value="<?php echo $pseudo_admin; ?>" size="40">
					  </div>
                      <br clear="all">                                                
